...
-- |Return a color triplet based on hue (in degrees) and, optionally, value
-- and saturation
doHSV :: [LispVal] -> IO (LispVal)
doHSV [h] = return $ makeFloatVector [r, g, b]
    where RGB r g b = hsv (extractFloat h) 1 1
doHSV [h, s, v] = return $ makeFloatVector [r, g, b]
    where RGB r g b = hsv (extractFloat h) (extractFloat s) (extractFloat v)
